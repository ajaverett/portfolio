...
We can measure how far a point is from the mean. The calculation is simple: "Temperature of a given day - Temperature average = How much the given day is different from the average" or more elegantly, $Y_i - \bar{Y}$. For example, if the temperature for a given day is 46 degrees, and the average temperature was 43, how much this day varies from the average is 3 degrees. $46 - 43 = 3$
...
$$
\begin{align*}
\text{Variance of the response variable} \\\;\\\\ =\frac{\sum_{i=1}^n (Y_i - \bar{Y})^2}{\text{degrees of freedom}} \\\;\\ =\frac{\text{SSTO}}{\text{degrees of freedom}}\\\;\\(\text{where degrees of freedom}=n-1 )
\end{align*}
$$
